...
<Note>
  O campo `price_cents` representa o valor bruto do fornecedor **em reais** (apesar do nome). Passe esse valor diretamente em `selectedVehicle.priceCents` ao criar o orçamento — o CGD converte para centavos internamente e aplica a margem. Veja [Orçamentos](/partner/quotes) para mais detalhes.
</Note>
